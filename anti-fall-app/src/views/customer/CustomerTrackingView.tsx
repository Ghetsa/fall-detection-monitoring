...
import { MapPin, Clock3, Activity, Smartphone, LocateFixed, Loader } from 'lucide-react';
        ...
        <div style={styles.heroCard}>
          <div>
            ...
            <p style={styles.heroText}>
              Lihat posisi terakhir lansia dan pastikan perangkat masih aktif serta terhubung dengan baik.
            </p>
          </div>

          <div style={styles.statusPill}>
            <span style={{ ...styles.statusDot, backgroundColor: device?.isOnline ? '#22c55e' : '#ef4444' }} />
        ...
        {lansiaList.length > 1 && (
          <div style={styles.selectorRow}>
            {lansiaList.map((l) => (
              <button
                key={l.id}
                style={{ ...styles.selectorBtn, backgroundColor: selected?.id === l.id ? '#2563eb' : '#ffffff', color: selected?.id === l.id ? '#ffffff' : '#334155', border: `1px solid ${selected?.id === l.id ? '#2563eb' : '#e2e8f0'}` }}
        ...
        <div style={styles.grid}>
          <div style={styles.mapCard}>
            <div style={styles.mapPlaceholder}>
              {deviceLoading ? (
                <div style={styles.mapPlaceholderInner}><Loader size={28} color="#1d4ed8" /></div>
              ) : device ? (
                <div style={styles.mapPlaceholderInner}>
                  <LocateFixed size={34} />
                  <span style={styles.mapText}>{device.locationName}</span>
                ...
                <div style={styles.mapPlaceholderInner}>
                  <LocateFixed size={34} />
                  <span style={styles.mapText}>Belum ada data lokasi</span>
                </div>
              )}
            </div>
          </div>
              ...
              <>
                <div style={styles.infoItem}>
                  <div style={styles.infoLeft}>
                    <span style={styles.iconWrap}><Activity size={18} /></span>
                ...
                <div style={styles.infoItem}>
                  <div style={styles.infoLeft}>
                    <span style={styles.iconWrap}><Clock3 size={18} /></span>
                    <span style={styles.infoLabel}>Diperbarui Terakhir</span>
                  </div>
                  <span style={styles.infoValue}>{device ? timeAgo(device.lastSeen) : '—'}</span>
                </div>

                <div style={styles.infoItem}>
                  <div style={styles.infoLeft}>
                    <span style={styles.iconWrap}><Smartphone size={18} /></span>
                    <span style={styles.infoLabel}>Kondisi Perangkat</span>
                  </div>
                  <span style={device?.isOnline ? styles.infoBadgeOnline : styles.infoBadgeOffline}>
                ...
                {device && (
                  <div style={styles.detailBox}>
                    ...
                    <p style={styles.detailText}>Koordinat: {device.latitude.toFixed(4)}, {device.longitude.toFixed(4)}</p>
...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  statusPill: { display: 'inline-flex', alignItems: 'center', gap: '10px', backgroundColor: '#ffffff', color: '#166534', padding: '12px 18px', borderRadius: '999px', fontWeight: 800, fontSize: '15px' },
  ...
  mapCard: { backgroundColor: '#ffffff', borderRadius: '22px', padding: '18px', boxShadow: '0 10px 25px rgba(15,23,42,0.05)', border: '1px solid #e2e8f0' },
  mapPlaceholder: { width: '100%', minHeight: '420px', borderRadius: '18px', background: 'linear-gradient(135deg, #dbeafe 0%, #bfdbfe 50%, #93c5fd 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  mapPlaceholderInner: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px', color: '#1d4ed8' },
  mapText: { fontSize: '18px', fontWeight: 800, textAlign: 'center' },
  mapCoords: { fontSize: '13px', fontWeight: 600, color: '#2563eb' },
  ...
  infoItem: { display: 'flex', justifyContent: 'space-between', gap: '16px', padding: '14px 0', borderBottom: '1px solid #e2e8f0', flexWrap: 'wrap', alignItems: 'center' },
  infoLeft: { display: 'flex', alignItems: 'center', gap: '10px' },
  iconWrap: { width: '32px', height: '32px', borderRadius: '10px', backgroundColor: '#eff6ff', color: '#1d4ed8', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  infoLabel: { fontSize: '14px', color: '#64748b' },
  infoValue: { fontSize: '15px', fontWeight: 700, color: '#0f172a' },
  infoBadgeOnline: { backgroundColor: '#dcfce7', color: '#166534', padding: '8px 12px', borderRadius: '999px', fontSize: '13px', fontWeight: 700 },
  infoBadgeOffline: { backgroundColor: '#fee2e2', color: '#b91c1c', padding: '8px 12px', borderRadius: '999px', fontSize: '13px', fontWeight: 700 },
  detailBox: { marginTop: '14px', backgroundColor: '#eff6ff', borderRadius: '16px', padding: '14px 16px' },
